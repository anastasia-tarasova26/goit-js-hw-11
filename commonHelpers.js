import { S as h, i as g } from "./assets/vendor-5b791d57.js";
(function () {
...
y.addEventListener("submit", (s) => {
  s.preventDefault();
  const o = new h(".gallery-photo a");
  (u.innerHTML = ""),
    d.value
      ? (c.classList.remove("hidden"),
        p.set("q", `${d.value}`),
        fetch(`${n.API_URL}?${p}`, { headers: { Accept: "application/json" } })
          .then((r) => {
            if (r.ok) return r.json();
            throw (c.classList.add("hidden"), new Error(r.status));
          })
          .then((r) => {
            r.hits.length
              ? (u.insertAdjacentHTML("beforeend", R(r.hits)), o.refresh())
              : l(n.ERROR_MESSAGES.IMAGES_NOT_FOUND),
              c.classList.add("hidden");
          })
          .catch((r) => {
            console.debug(r), l(n.ERROR_MESSAGES.RESOURSE_ERROR);
          }))
      : l(n.ERROR_MESSAGES.EMPTY_INPUT);
});
function R(s) {
  return s
    .map((o) => {
